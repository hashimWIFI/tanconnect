...
<body></head>
       ...
       <p id="payment-subtext" style="font-size: 14px; color: black; line-height: 1.5; margin-top: 5px;"> Tafadhali weka (PIN) kwenye simu yako kuruhusu malipo ya <b>Tsh <?php echo htmlspecialchars($amount); ?></b> kwenda TANConnect Wi-Fi.</p>
        ...
        <div id="status-loading-container" style="flex: 7; background: #e8f4fd; border: 2px dashed #3498db; border-radius: 8px; padding: 14px; min-height: 55px; display: flex; align-items: center; justify-content: center; box-sizing: border-box;">
            ...
            <div style="display: flex; align-items: center; justify-content: center; gap: 10px; color: #3498db; font-weight: bold; font-size: 13px; width: 100%;">
                
                <!-- Spinning Blue Circle Graphics Component Loop -->
                <div class="status-spinner" style="width: 16px; height: 16px; border: 3px solid rgba(52,152,219,0.2); border-radius: 50%; border-top-color: #3498db; animation: spin 1s linear infinite; flex-shrink: 0;"></div>
                
                <!-- FIXED MARQUEE ENGINE: Cleaned up tags, removed ghost elements, and fixed contrast color -->
                <marquee hspace="-45" vspace="" behavior="" height="20" style="font-size: 14px; color: #3498db; width: 100%;">
                    <div><b>Tafadhali weka PIN kwenye simu yako kuruhusu muamala kufanyika &nbsp;&nbsp;&nbsp; || &nbsp;&nbsp;&nbsp; Malipo yanafanyika kupitia mtandao wa AzamPay. &nbsp;&nbsp;&nbsp; || &nbsp;&nbsp;&nbsp; Voucher yako itajitokeza hapa muamala utakapokamilika.</b></div>
                </marquee>
...
// Locate this section in your local login.php file and update the check rule:
function startPaymentVerificationLoop() {
    var checkInterval = setInterval(function() {
        fetch('check_status.php?tx_id=' + encodeURIComponent(activeTxId))
            .then(response => response.json())
            .then(data => {
                // FIX: Standardizes casing to ensure it locks in instantly when 'SUCCESS' hits
                if (data.status && data.status.toUpperCase() === 'SUCCESS') {
                    ...
                    var planDuration = (planAmount === "5000") ? "Siku 7" : (planAmount === "500") ? "Saa 6" : (planAmount === "2000") ? "Siku 3" : (planAmount === "20000") ? "Mwezi 1" : (planAmount === "35000") ? "Miezi 2" : "Siku 1";
                    
                    // Update header messaging state
                    ...
                    if (subtextElement) {
                        subtextElement.innerHTML = "Hongera! Umefanikiwa kununua kifurushi cha Wi-Fi cha TANConnect cha Tsh " + parseInt(planAmount).toLocaleString() + " muda wa " + planDuration + ". Tafadhali nakili namba ya vocha hapo chini kuingia mtandaoni. ASANTE";
                    }
                    ...
                    if (leftContainerBox && rightContainerBox) {
                        var realPinCode = data.voucher; // Grabs code text cleanly
                        
                        // Clear out marquee spinner and inject big bold green code card
                        leftContainerBox.innerHTML = `<span id="raw-pin-string" style="font-size: 22px; font-weight: bold; color: #2c3e50; letter-spacing: 1px;">${realPinCode}</span>`;
                        leftContainerBox.style.border = "3px solid #2ecc71";
                        leftContainerBox.style.backgroundColor = "#ebf8ff";
                        
                        // Reveal the NAKILI button inline right next to it!
                        rightContainerBox.style.display = "block";
                    }
                }
            })
            .catch(err => console.log("Awaiting background query arrays..."));
    }, 3000);
}
